...
show_biz(), show_life(), show_mob(), show_crew(), show_ent(), show_main();
...
// main big img 슬라이드 모션
    ...
    function show_main() {
        let current = 0;
        let banner = $(".wrap_big_5>li");
            
        setInterval(function() {

        let prev = $(".wrap_big_5>li").eq(current);
        move(prev, 0, '-100%');

        current++;
        if(current == $(".wrap_big_5>li").length){
        current=0;
        }
        let next = $(".wrap_big_5>li").eq(current);
        move(next, '100%',0);  
...
function show_ent() {
    let current = 0;
    let banner = $(".mob6_small>li");
        
    setInterval(function() {

    let prev = $(".mob6_small>li").eq(current);
    move(prev, 0, '-100%');

    current++;
    if(current == $(".mob6_small>li").length){
    current=0;
    }
    let next = $(".mob6_small>li").eq(current);
    move(next, '100%',0);  
